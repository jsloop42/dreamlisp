...
- (NSString *)readableStringFor:(NSString *)string {
    @autoreleasepool {
        ...
        return [[NSString alloc] initWithFormat:@"\"%@\"", readable];
    }
}
...
- (nullable NSString *)printStringFor:(id<DLDataProtocol>)data readably:(BOOL)readably {
    @autoreleasepool {
        ...
        if ([DLSymbol isSymbol:data]) {
            ...
            return [[NSString alloc] initWithFormat:@"#<fault %@>", [(DLFault *)data value]];
        } else if ([DLLazySequence isLazySequence:data]) {
            return [[NSString alloc] initWithFormat:@"#<lazy-seq %@>", [(DLLazySequence *)data description]];
        } else if ([DLData isData:data]) {
            return [[NSString alloc] initWithFormat:@"#<data %@>", [(DLData *)data value]];
        } else if ([DLRegex isRegex:data]) {
            return [[NSString alloc] initWithFormat:@"<regex %@>", [(DLRegex *)data value]];
        } else if ([DLClass isClass:data]) {
            return [[NSString alloc] initWithFormat:@"#<class %@>", data];
        } else if ([DLObject isObject:data]) {
            return [[NSString alloc] initWithFormat:@"#<object %@>", data];
        } else if ([DLSlot isSlot:data]) {
            return [[NSString alloc] initWithFormat:@"#<slot %@>", data];
        } else if ([DLMethod isMethod:data]) {
            return [[NSString alloc] initWithFormat:@"#<method %@>", [data description]];
        }
