...
- (nullable NSMutableArray<id<DLDataProtocol>> *)readString:(NSString *)string {
    ...
    DLReader *reader = [[DLReader alloc] initWithTokens:tokens moduleName:_moduleName];
    ...
    @autoreleasepool {
        ...
        if ([token isEqual:@"("] || [token isEqual:@"["] || [token isEqual:@"{"]) {
            ...
            @autoreleasepool {
                ...
                return [[DLList alloc] initWithArray: @[[[DLSymbol alloc] initWithArity:2 position:0 string:@"with-meta" moduleName:[DLConst coreModuleName]],
...
- (void)symbolParseError:(NSString *)token {
    [[[DLError alloc] initWithFormat:DLSymbolParseError, token] throw];
}

- (DLSymbol *)symbolFromToken:(NSString *)token {
    if ([token isEqual:@"/"]) {
        return [[DLSymbol alloc] initWithArity:-1 string:@"/" moduleName:[DLConst coreModuleName]];
    }
...
- (nullable id<DLDataProtocol>)readAtom {
    ...
    if ([DLUtils matchString:token withExpression:_numExp]) {
        return [[DLNumber alloc] initWithString:token];
    } else if ([DLUtils matchString:token withExpression:_keywordExp]) {
        return [[DLKeyword alloc] initWithKeyword:token];
    } else if ([DLUtils matchString:token withExpression:_stringExp]) {
    ...
    } else if ([DLUtils matchString:token withExpression:_stringUnclosedExp]) {
